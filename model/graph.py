...
from heavyhex_circuits.heavyhex_37q import CHECK_DEFS, Z_STABS  # noqa: E402
...
def _support(name):
    return set(CHECK_DEFS[name][1])


def _coord(name):
    return ANC_COORD[CHECK_DEFS[name][2]]
    ...
    def __init__(self, code="heavyhex", num_cycles=3):
        if code != "heavyhex":
            raise NotImplementedError(
                f"GraphBuilder: code '{code}' not supported yet — the "
                f"rotated surface code arrives with the rsc3 milestone.")
        self.code = code
        ...
        assert len(nodes) == num_detectors(C)
        self.nodes = nodes
        ...
        names = list(CHECK_AT)
        share = {(a, b): bool(_support(a) & _support(b))
...
def augment_features(features, final_bits, code="heavyhex"):
    """(B, 2C, 4, 5) syndrome tensor + 17 final bits -> (B, 2C+1, 4, 5).

    The extra channel holds the 8 final-Z detector bits (support parity
    of final_bits XOR last-cycle Z-check plane) at the Z-ancilla cells.
    ...
    if code != "heavyhex":
        raise NotImplementedError(
            f"augment_features: code '{code}' not supported yet.")
    ...
    for j in Z_POS:
        ...
        for qp in CHECK_DEFS[name][1]:
            par = torch.bitwise_xor(par, bits[:, DIDX[qp]])
...
if __name__ == "__main__":
    gb = GraphBuilder("heavyhex", 3)
    deg = gb.adjacency.sum(1)
    print(f"nodes={gb.num_nodes} edges={int(gb.adjacency.sum() // 2)} "
          f"deg min/mean/max = {deg.min():.0f}/{deg.mean():.1f}/{deg.max():.0f}")
    kinds = [k for k, _, _ in gb.nodes]
    print(f"Z={kinds.count('Z')} X={kinds.count('X')} "
