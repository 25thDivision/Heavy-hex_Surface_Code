...
def npz_path(data_dir, noise, split, cycles, p, error_type):
    return (Path(data_dir) / noise_tag(noise)
            / f"{split}_d{DISTANCE}_c{cycles}_p{p}_{error_type}.npz")


def load_split(data_dir, noise, split, cycles, p, error_type, device="cpu"):
    """npz -> (features, y_qubit, y_logical) torch tensors (uint8, on device)."""
    path = npz_path(data_dir, noise, split, cycles, p, error_type)
    if not path.exists():
