...
**surface(miami)도 동일하게 자동 배치**야 — 회로·clbit·logical 정의가
전부 패치-로컬 좌표라 어떤 동형 배치든 의미가 보존돼 (miami 정적 자리의
data (1,5)=q84가 죽어 있던 문제가 이걸로 해결됨). 캘리브레이션이 죽은
값(None/1.0)을 보고하면: 프로파일 생성은 **유효 run만 평균**하고 전 run
불량인 키는 stim-안전 상한(readout/1Q 0.5, 2Q 0.75)으로 클램프 + 경고,
qpu Stim 빌더도 같은 상한으로 한 번 더 방어해 (DEM이 p≥15/16을 거부해
파이프라인이 죽는 사고 방지).
