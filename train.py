...
def parse_args():
    ...
    ap.add_argument("--mixed-rates",nargs="+",type=float,default=None,
                    help="mix multiple p datasets into one training set, e.g. 0.005 0.01 0.05")
    ap.add_argument("--mixed-noises",nargs="+",default=None,
                    help="mix multiple noise profiles into one training set")
...
def train_mixed(args, mod, noise, rates, et, device):
    print(f"\n{'=' * 70}")
    print(f">>> MIXED {noise} | p={rates} | {et} | cycles={args.cycles}")
    ...
    for p in rates:
        ...
        Xtr_all.append(Xtr)
        yqtr_all.append(yqtr)
        yltr_all.append(yltr)

    Xtr = torch.cat(Xtr_all, dim=0)
    yqtr = torch.cat(yqtr_all, dim=0)
    yltr = torch.cat(yltr_all, dim=0)

    print(f"    mixed train {tuple(Xtr.shape)} -> {device}")
    ...
    model = mod.HeavyHexCNN(
        in_channels=2 * args.cycles
    ).to(device)
    ...
    # mixed p에서는 하나의 p에 기반한 pos_weight를 쓰기 애매하므로
    # 처음 실험에서는 None 권장
    pos_weight = None

    # validation은 각 p별로 따로 만들어 둔다.
    val_loaders = {}

    for p in rates:
        Xva, yqva, ylva = load_split(
            args.data_dir,
            noise,
            "test",
            args.cycles,
        ...
        val_loaders[p] = FastTensorDataLoader(
            Xva,
            yqva,
            ylva,
            batch_size=args.batch_size
    ...
    rate_tag = "-".join(str(p) for p in rates)
    ckpt_path = (
        ckpt_dir
        / f"CNN_d{DISTANCE}_c{args.cycles}_mixed_p{rate_tag}_{noise_tag(noise)}.pt"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for p, loader in val_loaders.items():
            metrics[p] = evaluate(
                model,
                loader,
                mod,
        ...
        # model selection용으로 p별 LER 평균 사용
        mean_ler = float(
            np.mean([m["ler"] for m in metrics.values()])
        )

        print(
            f"    [Ep {epoch:02d}] "
            f"loss {train_loss:.4f} | "
            f"mean LER {mean_ler:.4f}"
        )
    ...
    print(
        ...
        f"mean validation LER={best_ler:.4f}"
    )

    print(f"checkpoint -> {ckpt_path}")

def train_mixed_noises(args, mod, noises, p, et, device):
    print(f"\n{'=' * 70}")
    print(f">>> MIXED NOISES {noises} | p={p} | {et} | cycles={args.cycles}")
    ...
    for noise in noises:
        ...
        val_loaders[noise] = FastTensorDataLoader(
            Xva,
            yqva,
            ylva,
            batch_size=args.batch_size
    ...
    noise_tag_mixed = "mixed_noise"
    ckpt_path = (
        Path(args.ckpt_dir)
        / f"CNN_d{DISTANCE}_c{args.cycles}_p{p}_{noise_tag_mixed}.pt"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for noise, loader in val_loaders.items():
            metrics[noise] = evaluate(
                model,
                loader,
                mod,
                args.aux_weight,
